...
Build the digital equivalent of a tournament paper scoreboard for backgammon. One thing, done well. The app already exists (React Native, iOS, landscape-only) with core scoring and Crawford rule tracking. This release adds: decrease-point via long press on a player's side (decreases that player's score by 1, repeatable, handles Crawford reversal); match length cycling via tap (forward) and long press (backward) on the center button at 0-0, no confirmation needed; immediate haptic feedback on touch-down using iOS Taptic Engine (`selection` preset via `react-native-haptic-feedback`) with a distinct `notificationSuccess` pattern on match win; per-game tally marks below each score (deferred to backlog); a subtle score animation (bounce/scale) when a point is added; and silent state persistence so match state is fully restored after force-quit. Also add `adjustsFontSizeToFit` to score text as a safety net for smaller devices.
